...
global {
	...
	int CELL_SIZE <- 20#m; // for archetypes overflow_coast and estuary_coast this value is 20#m  
	//int CELL_SIZE <- 10#m; // for archetype cliff_coast this value is 10#m   
	
	int NCOLS <- 730 ;   // NCOLS of the DEM in input and out 
	int NROWS <- 472 ;  // NROWS of the DEM in input and out
	float min_elevation_when_erasing_coastal_defenses <- 0.1#m; // if set to 0, this parameter is not used.
	...
	shape_file coastal_defenses_file <-shape_file("includes/raw_files/coastal_defenses - extended CoastalDef to remove from DEM.shp");
	...
	action erase_coastal_defenses
	{
		...
		if min_elevation_when_erasing_coastal_defenses = 0
		{
			...
			{
				list<mnt> tmp <-self.neighbors;
				elevation <- max([min_elevation_when_erasing_coastal_defenses,mean((tmp where(each.is_overlap_by_defense = false) collect(each.elevation)))]);
			}
		}
