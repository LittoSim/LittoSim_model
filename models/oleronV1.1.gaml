...
species ouvrage{
	...
	init {
		...
		if (height = 0.0){
			...
			height <- height / 100;
		} // CONVERSION: la table du shp est en centimètre, alors que dans lisflood ainsi que dans le modèle gama on est en mètre.
		if (length = 0) {
			length <- (max([1,(length(cells) - 1)]) * 20);
		} // ESTIMATION Longueur : 20 mètre fois le nb de cells traversés moins une.
	...
	//Calcule du moment ou il va y avoir rupture de digue
	action calcRupture {
		float p <- 0.0;
	...
	//La commune contruit une digure
	action new_dam_by_commune (int a_commune_id) {
		ask cell overlapping new_digure {
...
/* 
species digue {
	geometry myshape init:nil;
	
	aspect base
	{
		draw myshape color: #green;
	}
	
	reflex createDigue when: every(2){
		ask one_of(commune){
			point p1 <- any_location_in(self);
			point p2 <- any_location_in(self);
		}
	myshape <- polyline([commune.p1,commune.p2]);
	}
