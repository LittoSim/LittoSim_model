 ...
 * 
 *  L'idée est de conserver dans ce fichier l'ensemble des paramètres qui seront modifiable pour addapater le modèle à d'autres environnements
 *  Les variables qui n'ont pas vocation à être modifier resterons dans le ficher oleronV2.X.gaml 
 *  De cette manière on réduira les risques d'introduction de bug
 * 
 *  Cela pose la question de l'arbitrage des variables qui reste ici ou pas 
 */
...
global  {
	...
	// Gestion du temps dans les logs
	string LOG_FILE_NAME <- "log_"+machine_time+"csv";
	float START_LOG <- machine_time;
	
	// chemin d'accès a listflood sur la machine
	...
	// Les unité d'aménagement (PLU) sont codé en dure ici
...
//	int ACTION_DIKE_RAISED <- 18;
	 ...
	 *  Cela intervient de manière récurante durant la simulation.
	 * La récurance peut être gérer par la variable 'cycle_sauver
	 */
